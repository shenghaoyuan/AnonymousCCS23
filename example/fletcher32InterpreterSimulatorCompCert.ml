...
let fletcher32_init_regs =
  { r0_val = val64_zero; r1_val = (Vlong (Int64.repr 0)); r2_val = (Vlong
    (Int64.repr ((fun p->2*p) ((fun p->2*p) ((fun p->1+2*p) ((fun p->2*p)
    ...
    val64_zero; r10_val = (Vlong
    (Int64.repr ((fun p->2*p) ((fun p->2*p) ((fun p->2*p) ((fun p->1+2*p)
      ((fun p->2*p) ((fun p->1+2*p) ((fun p->1+2*p) ((fun p->1+2*p)
...
let fletcher32_init_bpf_state =
  { pc_loc = Int.zero; flag = BPF_OK; regs_st = fletcher32_init_regs;
    mrs_num = (Stdlib.succ (Stdlib.succ 0)); bpf_mrs =
    fletcher32_init_memory_regions; ins_len = (length test_fletcher32_int64);
    ins = test_fletcher32_int64; bpf_m = mem2 }
