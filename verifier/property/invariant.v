...
Proof.
  ...
  - assert (Hlt: (k <= ins_len st)%nat). {
    ...
    unfold is_well_dst'.
    unfold state.eval_ins, ListAsArray.List64AsArray.index.
    rewrite H in Hlt.
    assert (Heq: (Z.to_nat (Int.unsigned (Int.repr (Z.of_nat k))))
      = k). {
    ...
    destruct List.nth_error eqn: Hnth.
    2:{
    ...
    assert (Haux2:= bpf_verifier_aux2_some).

    specialize (Haux2 st k (ins_len st) (BinrBPF.get_opcode i) i).
...
Proof.
  ...
  assert (Hst_eq: st = s). {
    ...
    - unfold eval_ins, is_well_dst, get_opcode, bindM, returnM in Haux.
      ...
      destruct state.eval_ins in Haux; [| inversion Haux].
...
Proof.
  ...
  eapply bpf_verifier_aux_some in H0 as Haux; eauto.
  ...
  - unfold eval_ins.
    ...
    unfold state.eval_ins, ListAsArray.List64AsArray.index.
    assert (Heq: (Z.to_nat (Int.unsigned (Int.repr (Z.of_nat (ins_len st - 1)))) =
      (ins_len st - 1))%nat). {
...
Proof.
  ...
  - assert (Hlt: (k <= ins_len st)%nat). {
    ...
    unfold state.eval_ins in H1.
    destruct ListAsArray.List64AsArray.index eqn: Hins; [| inversion H1].
    ...
    rewrite Haux2.
    assumption.
    assert (Hopcode:= get_opcode_le_255).
    specialize (Hopcode i).
    lia.
Qed.
...
Proof.
  ...
  destruct Int.cmpu; inversion H1.
  unfold state.eval_ins in H3.
  destruct ListAsArray.List64AsArray.index; inversion H3.
  reflexivity.
Qed.
