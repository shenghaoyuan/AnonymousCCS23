...
Definition eval_flag: M state val := fun st => 
  match eval_flag st with
  | Some f => Some (f, st)
  | None => None
  end.

Definition upd_flag (f:val) : M state unit := fun st =>
...
Definition eval_reg (r: reg) : M state val := fun st =>
...
Definition cmp_ptr32_nullM (v: val): M state bool := fun st =>
  match cmp_ptr32_null (ConcreteState.eval_mem st) v with
...
Axiom lemma_exec_function0 :
  forall b ofs st1,
      exists v st2, exec_function (Vptr b ofs) st1 = Some (Vint v, st2) /\ cmp_ptr32_null (ConcreteState.eval_mem st1) (Vptr b ofs) = Some false.
