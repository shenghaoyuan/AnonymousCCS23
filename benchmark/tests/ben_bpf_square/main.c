...
unsigned char square_compcert_bpf_bin[] = {
...
int main(void){  
  ...
  struct bpf_state st = {
    .state_pc = 0,
    .regsmap = {0LLU, 156LLU, 0LLU, 0LLU, 0LLU, 0LLU, 0LLU, 0LLU, 0LLU, 0LLU, (uintptr_t)_bpf_stack+512},
    .bpf_flag = vBPF_OK,
    .mrs = memory_regions,
    .mrs_num = ARRAY_SIZE(memory_regions),
    .ins = (unsigned long long *) square_compcert_bpf_bin,
    .ins_len = sizeof(square_compcert_bpf_bin),
  };
#elif defined(MODULE_GEN_IBPF)
  jitted_thumb_list = ibpf_state.jitted_thumb_list;
  ibpf_full_state_init(&ibpf_state, 1);
  ibpf_set_code(&ibpf_state, square_compcert_bpf_bin, sizeof(square_compcert_bpf_bin));
  ibpf_set_input(&ibpf_state, 156LLU, 0LLU, 0LLU, 0LLU, 0LLU);
  jit_alu32(&ibpf_state.st);
#else
  bpf_t bpf = {
    .application = (uint8_t*)&square_compcert_bpf_bin,
    .application_len = sizeof(square_compcert_bpf_bin),
    .stack = _bpf_stack,
