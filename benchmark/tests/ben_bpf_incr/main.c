...
unsigned char incr_compcert_bpf_bin[] = {
...
int main(void){  
  ...
  struct bpf_state st = {
    ...
    .ins = (unsigned long long *) incr_compcert_bpf_bin,
    .ins_len = sizeof(incr_compcert_bpf_bin),
  };
#elif defined(MODULE_GEN_IBPF)
  jitted_thumb_list = ibpf_state.jitted_thumb_list;
  ibpf_full_state_init(&ibpf_state, 1);
  ibpf_set_code(&ibpf_state, incr_compcert_bpf_bin, sizeof(incr_compcert_bpf_bin));
  ibpf_set_input(&ibpf_state, 156LLU, 0LLU, 0LLU, 0LLU, 0LLU);
  jit_alu32(&ibpf_state.st);
#else
  bpf_t bpf = {
    .application = (uint8_t*)&incr_compcert_bpf_bin,
    .application_len = sizeof(incr_compcert_bpf_bin),
    .stack = _bpf_stack,
