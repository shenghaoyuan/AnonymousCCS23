...
Fixpoint bpf_verifier_aux (pc len: nat) (st: state.state): bool :=
    ...
    | O => true
    | S n =>
      match List64AsArray.index (ins st) (Int.repr (Z.of_nat n)) with
      | Some i =>
      ...
      if negb
       (Int.ltu (Int.divu (Int.repr Int.max_unsigned) (Int.repr 8))
        ...
        if bpf_verifier_aux len len st then
          match List64AsArray.index (ins st) (Int.repr (Z.of_nat  (len - 1))) with
          | Some i => Int64.eq i (Int64.repr 0x95)
          | None => false
          end
        else
          false
...
Lemma bpf_verifier_aux_implies:
  forall st pc op ins k
    ...
    (Hins: Some ins = List64AsArray.index (state.ins st) (Int.repr (Z.of_nat pc))),
...
Proof.
  ...
  - assert (Hpc_eq: pc = k \/ pc < k) by lia.
    ...
    + apply IHk; auto.
      simpl in Hst.
      destruct List64AsArray.index; [| inversion Hst].
      match goal with
      | H: (if ?X then _ else _) = _ |- _ =>
        destruct X eqn: Haux2;[ | inversion H]
      end.
...
Lemma bpf_verifier_aux_implies':
  forall st pc op ins k
    ...
    (Hpc: pc < k)
    (Hop: op = get_opcode ins)
    (Hins: Some ins = List64AsArray.index (state.ins st) (Int.repr (Z.of_nat (k-1-pc)))),
