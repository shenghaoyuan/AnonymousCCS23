...
_NB: The branch `jit-arm-mode` is a early version that supports arm32 ARM-ISA (A1 encoding), while the branch `jit` adopts arm32 Thumb-ISA because RIOT-OS only supports thumb mode for cortex-M support board_
