...
(Sreturn (Some (Efield
                 (Ederef (Etempvar _st (tptr (Tstruct _bpf_state noattr)))
                   (Tstruct _bpf_state noattr)) _bpf_flag tuint)))
|}.

Definition f_upd_flag := {|
  fn_return := tvoid;
  fn_callconv := cc_default;
  fn_params := ((_st, (tptr (Tstruct _bpf_state noattr))) :: (_f, tuint) ::
                nil);
  fn_vars := nil;
  fn_temps := nil;
  fn_body :=
(Ssequence
  (Sassign
    (Efield
      (Ederef (Etempvar _st (tptr (Tstruct _bpf_state noattr)))
        (Tstruct _bpf_state noattr)) _bpf_flag tuint) (Etempvar _f tuint))
  (Sreturn None))
|}.
...
(Composite _memory_region Struct
 ...
 Composite _bpf_state Struct
   (Member_plain _state_pc tuint :: Member_plain _bpf_flag tuint ::
